...
/**
 * VentanaAgregarPuntoInteres — Flujo integrado de mapa (igual a VentanaAgregarNodo)
 *
 * Pasos:
 * 1. PASO_MAPA        → Panel de mapa interactivo (clic derecho coloca el pin).
 *                       Muestra nodos (negro + borde naranja) y POIs (naranja + borde negro).
 * 2. PASO_DATOS       → Nombre, horario, descripción y ciclovía.
 * 3. PASO_CONFIRMACION → Resumen antes de guardar.
 */
public class VentanaAgregarPuntoInteres extends JFrame {
    ...
    // Constructor
    // =============================================================================
    public VentanaAgregarPuntoInteres(Usuario usuarioActivo, GestorUsuarios gestor,
    ...
    // =============================================================================
    private void initUI() {
        setTitle("VelóGRID — Agregar Punto de Interés");
    ...
    private JPanel buildTopBar() {
        ...
        lblTitulo = new JLabel("NUEVO PUNTO DE INTERÉS — Paso 1: Ubicación en el mapa");
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // PASO 2 — Datos del punto de interés
    // ─────────────────────────────────────────────────────────────────────────────
    private JPanel buildPasoDatos() {
        ...
        JLabel lblTitle = new JLabel("Información del Punto de Interés");
        ...
        JLabel lblSub = new JLabel("Ingresa el nombre, horario, descripción y si tiene ciclovía.");
        ...
        // Nombre
        card.add(makeFieldLabel("Nombre / Local:"));
        card.add(Box.createVerticalStrut(6));
        ...
        // Horario
        card.add(makeFieldLabel("Horario (ej: 08:00–20:00):"));
        card.add(Box.createVerticalStrut(6));
        txtHorario = makeTextField();
        txtHorario.setMaximumSize(new Dimension(Integer.MAX_VALUE, 36));
        txtHorario.setAlignmentX(Component.LEFT_ALIGNMENT);
        card.add(txtHorario);

        card.add(Box.createVerticalStrut(18));

        // Descripción
        card.add(makeFieldLabel("Descripción:"));
        card.add(Box.createVerticalStrut(6));
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // PASO 3 — Confirmación (construido dinámicamente al llegar)
    // ─────────────────────────────────────────────────────────────────────────────
    private JPanel buildPasoConfirmacion() {
        ...
        JLabel lblTitle = new JLabel("¿Confirmar este Punto de Interés?");
        ...
        card.add(makeResumenFila("Horario:",     horarioPendiente));
        card.add(Box.createVerticalStrut(12));
        card.add(makeResumenFila("Descripción:", descripcionPendiente));
        card.add(Box.createVerticalStrut(12));
        ...
        JButton btnGuardar = new JButton("✓  Guardar punto");
        ...
        btnGuardar.addActionListener(e -> guardarPuntoInteres());
        ...
        switch (pasoActual) {
            case MAPA:
                if (!panelMapa.tienePinColocado()) {
                    mostrarError("Haz clic derecho en el mapa para seleccionar la ubicación del punto.");
                    return;
                }
            ...
            case DATOS:
                ...
                if (nombre.isEmpty()) {
                    mostrarError("Por favor ingresa un nombre para el punto de interés.");
                    return;
                }
                nombrePendiente      = nombre;
                horarioPendiente     = txtHorario.getText().trim();
                descripcionPendiente = txtDescripcion.getText().trim();
    ...
    private void irAPaso(Paso nuevoPaso) {
        ...
        switch (nuevoPaso) {
            case MAPA:
                lblTitulo.setText("NUEVO PUNTO DE INTERÉS — Paso 1: Ubicación en el mapa");
            ...
            case DATOS:
                lblTitulo.setText("NUEVO PUNTO DE INTERÉS — Paso 2: Datos");
                btnAccionTopBar.setText("Continuar >");
                btnAccionTopBar.setVisible(true);
            ...
            case CONFIRMACION:
                lblTitulo.setText("NUEVO PUNTO DE INTERÉS — Paso 3: Confirmar");
    ...
    // Guardado final
    // =============================================================================
    private void guardarPuntoInteres() {
        try {
            ...
            gestorNodos.getMapaNodos().put(nuevoId, poi);
            gestorNodos.guardarPuntosInteresCSV("baseDeDatos/puntosInteres.csv");

            JOptionPane.showMessageDialog(this,
                    "✓  Punto de Interés \"" + nombrePendiente + "\" registrado con ID " + nuevoId + ".",
                    "Punto guardado",
            ...
            mostrarError("No se pudo guardar el punto: " + ex.getMessage());
        }
    }

    // =============================================================================
    // Panel de mapa — nodos (negro+naranja) y POIs (naranja+negro)
    // =============================================================================
    private static class PanelMapa extends JPanel {
        ...
        private static final int RADIO_POI  = 9;  // un poco más grande para distinguir
        ...
        private void configurarNavegacion() {
            ...
            // Clic izquierdo (BUTTON1) → arrastrar mapa
        ...
        private void configurarInteraccion() {
            // Clic derecho (BUTTON3) → colocar pin
            addMouseListener(new MouseAdapter() {
        ...
        protected void paintComponent(Graphics g) {
            ...
            // Restaurar transformación para dibujar en coordenadas de pantalla (tamaño fijo)
            g2.setTransform(tx);
            ...
            for (Nodo nodo : gestorNodos.getMapaNodos().values()) {
                ...
                if (esPOI) {
                    // POI → relleno naranja + borde negro (inverso del nodo)
                    int r = RADIO_POI;
                    g2.setColor(new Color(252, 76, 2));       // naranja
                    g2.fillOval(screenX - r, screenY - r, r * 2, r * 2);
                    g2.setColor(new Color(15, 15, 20));        // negro VeloGrid
                    g2.setStroke(new BasicStroke(2f));
                    g2.drawOval(screenX - r, screenY - r, r * 2, r * 2);
                } else {
                    // Nodo normal → relleno negro + borde naranja
                    int r = RADIO_NODO;
                    g2.setColor(new Color(15, 15, 20));        // negro VeloGrid
                    g2.fillOval(screenX - r, screenY - r, r * 2, r * 2);
                    g2.setColor(new Color(252, 76, 2));        // naranja
                ...
                g2.setColor(esPOI ? new Color(252, 76, 2) : new Color(240, 240, 240)); // naranja para POI, blanco para nodo
    ...
    // Helpers de diseño
    // =============================================================================
    private JLabel makeFieldLabel(String texto) {
